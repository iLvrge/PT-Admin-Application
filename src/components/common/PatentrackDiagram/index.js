...
class PatentrackDiagram extends React.Component {
  ...
  constructor(props_) {
    ...
    console.log('%cPatentrack Diagram by Vladimir V. KUCHINOV, v. 0.99', 'color: #494949; font-size: 12px; font-family: sans-serif;');
  ...
  parseData(){
      ...
      this.props.data.box.forEach((box_, i_) => {
          ...
          if(box_.segment == 0) { nodes.inventors.push(box_); this.data.indices[box_.id] = { type: 'inventors', i: nodes.inventors.length - 1 }; }
      ...
      nodes.inventors.forEach((inventor_, i_) => {
              ...
              this.data.inventors.push({
                  ...
                  executionDate: inventor_.execution_date,
                  recordedDate: inventor_.recorded_date,
                  document: null,
                  type: 0,
                  left: [],
                  right: [],
      ...
      nodes.owners.forEach((owner_, i_) => {
         ...
         this.data.owners.push({
          ...
          //console.log(owner_, nodes.owners[i_ + 1])
          if((i_ + 1) < nodes.owners.length && nodes.owners[i_ + 1].date_1 == owner_.date_1) { console.log('on the same row', owner_); dx++; offsets.banks = dx + 1; }
      ...
      this.height = (dheight + 1) * config.node.gap.y + config.node.topOffset;
      ...
      for(let i = 0; i < segments.length; i++){
          for(let j = 0; j < segments.length; j++){
              ...
              if(i != j){
                  ...
                  if(this.twoLinesIntersection(segments[i], segments[j])){ return true; break; }
  ...
  updateDiagram(event_){
        ...
        if(event_.currentTarget.getAttribute('type') == 'checkbox'){
           ...
           this.state.filters[event_.currentTarget.getAttribute('id')] = event_.currentTarget.checked;
  ...
  render () {
    ...
    return (
        ...
        <div id={'patentrackDiagramDiv'} ref={(divElement) => this.divElement = divElement}>
            <svg id={'svg_' + this.prefix} {...svgParams} key={this.generateHashhKet()} mlndes='http://www.w3.org/2000/svg' xmlnsXlink='http://www.w3.org/1999/xlink'>
         ...
         <PatentTopTitle width={this.props.parentWidth} titleTop={this.props.titleTop} title={this.parseTitle()}/>
         <PatentBottomUI width={this.props.parentWidth} update={this.updateDiagram} comment={this.props.comment} commentContent={this.props.data.comment} share={this.props.share} patent={this.props.data.general} colorScheme={config.colors} toolbarBottom={this.props.toolbarBottom} quantatives={{assignment: {current: this.state.assignments.length == 0 ? 0 : this.state.assignments[this.state.assignments.length - 1], total: this.state.limits.assignments}, assignee: {current: this.state.assignees.length == 0 ? 0 : this.state.assignees[this.state.assignees.length - 1] + 1, total: this.state.limits.assignees }}}/>
        </div>
